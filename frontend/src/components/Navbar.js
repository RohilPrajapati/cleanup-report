...
const Navbar = () => {
  ...
  const handleLogout = () => {
    ...
    logout_api_call(refreshToken)
      .then((res) => {
        // Clear local tokens or user data
        logout(); // assuming this clears localStorage/session
        // Navigate after API call is complete
        navigate('/login');
      })
      .catch((err) => {
        console.error("Logout API failed:", err);
        // Optionally still log out locally
        logout();
        navigate('/login');
      });
  };
